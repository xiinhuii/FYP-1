                                ...
                                if (task.isSuccessful()) {
                                    ...
                                    document.get().addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
                                        ...
                                        public void onSuccess(DocumentSnapshot documentSnapshot) {
                                            ...
                                            System.out.println("Check my status2222: " + sessionStarted);


                                            //System.out.println("Current Status :" + hasSession);
                                            SharedPreferences sharedPref_status = getSharedPreferences(SHARED_PREFS_STATUS, Context.MODE_PRIVATE);
                                            SharedPreferences.Editor editor_status = sharedPref_status.edit();

                                            editor_status.putBoolean("SHARED_STATUS", sessionStarted);
                                            editor_status.apply();



                                            intent = new Intent(getApplicationContext(), MainActivity.class);
                                            //intent.putExtra("status", sessionStarted);
                                            startActivity(intent);
                                            finish();
